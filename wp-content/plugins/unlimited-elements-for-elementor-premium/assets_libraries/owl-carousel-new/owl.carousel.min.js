...
! function(l, c, s, a) {
    ...
    function p(t, e) {
        this.settings = null, this.options = l.extend({}, p.Defaults, e), this.$element = l(t), this._handlers = {}, this._plugins = {}, this._supress = {}, this._current = null, this._speed = null, this._coordinates = [], this._breakpoint = null, this._width = null, this._items = [], this._clones = [], this._mergers = [], this._widths = [], this._invalidated = {}, this._pipe = [], this._drag = {
            time: null,
            target: null,
            pointer: null,
            stage: {
        ...
        }, this)), l.each(p.Plugins, l.proxy(function(t, e) {
            this._plugins[t.charAt(0).toLowerCase() + t.slice(1)] = new e(this)
        }, this)), l.each(p.Workers, l.proxy(function(t, e) {
            this._pipe.push({
                filter: e.filter,
    ...
    p.Defaults = {
        ...
        run: function() {
            this._width = Math.floor(this.$element.width()), h("set total width this._width: " + this._width)
        }
    }, {
        ...
        filter: ["width", "items", "settings"],
        run: function(t) {
            !0 === this.settings.debug && (d = !0), h("run!"), 1 == this.settings.items && 1 == this.settings.pixel_gap_fix && (this.settings.item_size_gap = 1, h("set items size gap: 1")), h("settings - stage padding type: " + this.settings.paddingType), h("settings - stage padding: " + this.settings.stagePadding), this.settings.paddingType && "none" != this.settings.paddingType || !this.settings.stagePadding || (this.settings.paddingType = "both", h("set stage padding type - both"));
            var e, i = this.getItemWidth();
            for (h("Item width: " + i), this.settings.stagePadding > i && (this.settings.stagePadding = 0, e = this.getItemWidth(), this.settings.stagePadding = Math.floor(.5 * e), i = this.getItemWidth(), h("set new stagePadding: " + this.settings.stagePadding + " and item width: " + i)), merge = null, iterator = this._items.length, grid = !this.settings.autoWidth, widths = [], t.items = {
                    merge: !1,
                    width: i
        ...
        run: function() {
            ...
            h("Set coordinates"), h(s), this._coordinates = s
        }
    }, {
        filter: ["width", "items", "settings"],
        run: function() {
            var t = this.settings.stagePadding,
                ...
                s = this.$stage.parent(),
                n = s.width();
            h("parent Outer Stage Width: " + s.width()), h("element width this.$element.width(): " + this.$element.width());
            var o = {
            ...
            h("set outer stage css"), h(n), "none" == i ? (this.$stage.css(o), s.width(n)) : "both" == i ? (this.$stage.css(r), s.css(n), h("Set stage width css (both)"), h(r)) : "left" == i ? (this.$stage.css(a), s.css(n), h("Set stage width css (left)"), h(a)) : "right" == i && (this.$stage.css(t), s.css(n), h("Set stage width css (right)"), h(t))
        }
    }, {
        filter: ["width", "items", "settings"],
        run: function(t) {
    ...
    }], p.prototype.initializeStage = function() {
    ...
    }, p.prototype.initializeItems = function() {
        ...
        this.$stage.children().each(function() {
            if (1 == n.settings.lazyLoad) return !1;
            var t, e = jQuery(this),
                i = e.find("img");
    ...
    }, p.prototype.optionsLogic = function() {
        this.settings.autoWidth && (this.settings.stagePadding = !1, this.settings.merge = !1)
    }, p.prototype.prepare = function(t) {
        ...
        var e;
        switch (t = t || p.Width.Default) {
            case p.Width.Inner:
            case p.Width.Outer:
                e = this._width, h("calc width for dimention: " + t);
            default:
                switch (this.settings.paddingType) {
                    case "left":
                    case "right":
                        h("calc width with stagePadding: " + this.settings.paddingType), e = this._width - this.settings.stagePadding + this.settings.margin;
                        break;
                    case "both":
                        h("calc width with stagePadding: " + this.settings.paddingType), e = this._width - 2 * this.settings.stagePadding + this.settings.margin;
                        break;
                    default:
                        e = this._width + this.settings.margin, h("calc width without stagePadding for dimention: " + t)
                }
        }
        return h("carousel width (no stagepadding and margins): " + e), e
    }, p.prototype.refresh = function() {
        h("---------------------refresh carousel-----------------------"), h("window width: " + c.innerWidth), h("element width: " + this.$element.width()), h("stage width: " + this.$stage.width()), this.enter("refreshing"), this.trigger("refresh"), this.setup(), this.optionsLogic(), this.$element.addClass(this.options.refreshClass), this.update(), this.$element.removeClass(this.options.refreshClass), this.leave("refreshing"), this.trigger("refreshed")
    }, p.prototype.onThrottledResize = function() {
        ...
        return h("---------------------resize carousel-----------------------"), this._items.length ? this.isVisible() ? (h("resizing"), this.enter("resizing"), this.trigger("resize").isDefaultPrevented() ? (this.leave("resizing"), !1) : (this.invalidate("width"), this.$stage.parent().css({
            width: ""
        }), this.refresh(), this.update(), this.leave("resizing"), void this.trigger("resized"))) : (h("not visible"), !1) : (h("same length"), !1)
    }, p.prototype.registerEventHandlers = function() {
        l.support.transition && this.$stage.on(l.support.transition.end + ".owl.core", l.proxy(this.onTransitionEnd, this)), !1 !== this.settings.responsive && this.on(c, "resize", this._handlers.onThrottledResize), this.settings.mouseDrag && (this.$element.addClass(this.options.dragClass), this.$stage.on("mousedown.owl.core", l.proxy(this.onDragStart, this)), this.$stage.on("dragstart.owl.core selectstart.owl.core", function() {
            return !1
    ...
    }, p.prototype.onDragMove = function(t) {
        var e, i = null,
            s = null,
    ...
    }, p.prototype.onDragEnd = function(t) {
        var e = this.difference(this._drag.pointer, this.pointer(t)),
    ...
    }, p.prototype.closest = function(i, s) {
        var n = -1,
            o = this.width(),
            r = this.coordinates();
        ...
        var e = 0 < this.speed();
        this.is("animating") && this.onTransitionEnd(), e && (this.enter("animating"), this.trigger("translate")), l.support.transition ? (h("Go to coordinate: " + t), this.$stage.css({
    ...
    }, p.prototype.current = function(t) {
    ...
    }, p.prototype.minimum = function(t) {
        return t ? 0 : this._clones.length / 2
    }, p.prototype.items = function(t) {
        return t === a ? this._items.slice() : (t = this.normalize(t, !0), this._items[t])
    }, p.prototype.mergers = function(t) {
        return t === a ? this._mergers.slice() : (t = this.normalize(t, !0), this._mergers[t])
    }, p.prototype.clones = function(i) {
        function s(t) {
    ...
    }, p.prototype.speed = function(t) {
        return t !== a && (this._speed = t), this._speed
    }, p.prototype.coordinates = function(t) {
        var e, i = 1,
            s = t - 1;
        return t === a ? l.map(this._coordinates, l.proxy(function(t, e) {
            return this.coordinates(e)
        }, this)) : (this.settings.center ? (this.settings.rtl && (i = -1, s = t + 1), e = this._coordinates[t], e += (this.width() - e + (this._coordinates[s] || 0)) / 2 * i) : e = this._coordinates[s] || 0, e = Math.ceil(e))
    }, p.prototype.duration = function(t, e, i) {
        return 0 === i ? 0 : Math.min(Math.max(Math.abs(e - t), 1), 6) * Math.abs(i || this.settings.smartSpeed)
    }, p.prototype.to = function(t, e) {
    ...
    }, p.prototype.prev = function(t) {
        t = t || !1, this.to(this.relative(this.current()) - 1, t)
    }, p.prototype.onTransitionEnd = function(t) {
        if (t !== a && (t.stopPropagation(), (t.target || t.srcElement || t.originalTarget) !== this.$stage.get(0))) return !1;
        this.leave("animating"), this.trigger("translated")
    }, p.prototype.viewport = function() {
        var t;
        return this.options.responsiveBaseElement !== c ? t = l(this.options.responsiveBaseElement).width() : c.innerWidth ? t = c.innerWidth : s.documentElement && s.documentElement.clientWidth ? t = s.documentElement.clientWidth : console.warn("Can not detect viewport width."), t
    }, p.prototype.replace = function(t) {
        this.$stage.empty(), this._items = [], t = t && (t instanceof jQuery ? t : l(t)), this.settings.nestedItemSelector && (t = t.find("." + this.settings.nestedItemSelector)), t && t.length && t.filter(function() {
            return 1 === this.nodeType
        }).each(l.proxy(function(t, e) {
            e = this.prepare(e), this.$stage.append(e), this._items.push(e), this._mergers.push(+e.find("[data-merge]").addBack("[data-merge]").attr("data-merge") || 1)
        }, this)), this.reset(this.isNumeric(this.settings.startPosition) ? this.settings.startPosition : 0), this.invalidate("items");
        t = this.settings.startPosition;
        l(this.$element).trigger("to.owl.carousel", t)
    ...
    }, p.prototype.preloadAutoWidthImages = function(t) {
        t.each(l.proxy(function(t, e) {
            this.enter("pre-loading"), e = l(e), l(new Image).one("load", l.proxy(function(t) {
                e.attr("src", t.target.src), e.css("opacity", 1), this.leave("pre-loading"), this.is("pre-loading") || this.is("initializing") || this.refresh()
            }, this)).attr("src", e.attr("src") || e.attr("data-src") || e.attr("data-src-retina"))
        }, this))
    }, p.prototype.isElementInViewport = function(t) {
        var e = t.offset().top,
            i = e + t.outerHeight(),
            s = jQuery(c).scrollTop(),
            t = s + jQuery(c).height();
        return s < i && e < t
    }, p.prototype.scrollToTop = function(t, e) {
        jQuery("html, body").animate({
            scrollTop: t + e
        }, 400)
    }, p.prototype.scrollToHead = function() {
        ...
        this.settings.progressScroll && (this._scroll = {
            lastScrollTop: 0,
            distanceScrolled: 0,
            lastDirection: null
        }, l(c).on("scroll.owl.progress", l.proxy(this.onScrollProgress, this)))
    }, p.prototype.onScrollProgress = function(t) {
        var e, i, s, n, o, r, a, h;
        !1 !== this.settings.progressScroll && (this.isTransitioning || this.isAnimating || this.isDragging ? this._scroll.lastScrollTop = l(c).scrollTop() : (e = l(c).scrollTop(), i = this._scroll.lastScrollTop, (n = l(c).width()) < 767 && this.settings.progressScrollDistanceMobile ? s = this.settings.progressScrollDistanceMobile : 767 <= n && n < 1024 && this.settings.progressScrollDistanceTablet ? s = this.settings.progressScrollDistanceTablet : 1024 <= n && this.settings.progressScrollDistance && (s = this.settings.progressScrollDistance), 0 != (o = e - i) && (r = Math.abs(o), n = i < e ? "down" : "up", (o = this._scroll.lastDirection !== n) && (this._scroll.distanceScrolled = 0, this._scroll.lastDirection = n), o && s < r && (r = 0, 1 == d && (a = '<div class="owl-debug" style="position: fixed; background: #960; color: #FFF; z-index: 1000; padding: 10px; left: 20px; top: 20px; border-radius: 4px; font-family: monospace; line-height: 1.4;">NaN' + n + '<span style="color: #FF0;">(SPIKE ZEROED)</span> <br>NaN' + Math.abs(e - i).toFixed(0) + "px <br>NaN" + s + "pxNaN", (h = jQuery(".owl-debug")).length ? h.html(a) : jQuery("body").append(a))), this._scroll.distanceScrolled += r, this._scroll.distanceScrolled >= s ? (r = "down" === n ? "next.owl" : "prev.owl", this.$element.trigger(r), this._scroll.distanceScrolled = 0, 1 == d && (a = '<div class="owl-debug" style="position: fixed; background: #060; color: #FFF; z-index: 1000; padding: 10px; left: 20px; top: 20px; border-radius: 4px; font-family: monospace; line-height: 1.4;">NaN' + n + '<span style="color: #FF0;">(TRIGGERED)</span> <br>NaN' + this._scroll.distanceScrolled.toFixed(0) + "px <br>NaN" + s + "px + </div>", (h = jQuery(".owl-debug")).length && h.remove(), jQuery("body").append(a))) : 1 == d && (a = '<div class="owl-debug" style="position: fixed; background: #666; color: #FFF; z-index: 1000; padding: 10px; left: 20px; top: 20px; border-radius: 4px; font-family: monospace; line-height: 1.4;">NaN' + n + "<br>NaN" + this._scroll.distanceScrolled.toFixed(0) + "px <br>NaN" + s + "pxNaN", (h = jQuery(".owl-debug")).length ? h.html(a) : jQuery("body").append(a)), this._scroll.lastScrollTop = e)))
    }, p.prototype.destroy = function() {
        for (var t in this.$element.off(".owl.core"), this.$stage.off(".owl.core"), l(s).off(".owl.core"), !1 !== this.settings.responsive && (c.clearTimeout(this.resizeTimer), this.off(c, "resize", this._handlers.onThrottledResize)), this._plugins) this._plugins[t].destroy();
        this.$stage.children(".cloned").remove(), this.$stage.unwrap(), this.$stage.children().contents().unwrap(), this.$stage.children().unwrap(), this.$stage.remove(), this.$element.removeClass(this.options.refreshClass).removeClass(this.options.loadingClass).removeClass(this.options.loadedClass).removeClass(this.options.rtlClass).removeClass(this.options.dragClass).removeClass(this.options.grabClass).attr("class", this.$element.attr("class").replace(new RegExp(this.options.responsiveClass + "-\\S+\\s", "g"), "")).removeData("owl.carousel"), l(c).off("scroll.owl.progress")
    }, p.prototype.op = function(t, e, i) {
    ...
    }, p.prototype.on = function(t, e, i, s) {
        t.addEventListener ? t.addEventListener(e, i, s) : t.attachEvent && t.attachEvent("on" + e, i)
    }, p.prototype.off = function(t, e, i, s) {
        t.removeEventListener ? t.removeEventListener(e, i, s) : t.detachEvent && t.detachEvent("on" + e, i)
    }, p.prototype.trigger = function(t, e, i, s, n) {
        var o = {
                item: {
    ...
    }, p.prototype.enter = function(t) {
        l.each([t].concat(this._states.tags[t] || []), l.proxy(function(t, e) {
            this._states.current[e] === a && (this._states.current[e] = 0), this._states.current[e]++
        }, this))
    }, p.prototype.leave = function(t) {
        l.each([t].concat(this._states.tags[t] || []), l.proxy(function(t, e) {
            this._states.current[e]--
        }, this))
    }, p.prototype.register = function(i) {
        var e;
        i.type === p.Type.Event ? (l.event.special[i.name] || (l.event.special[i.name] = {}), l.event.special[i.name].owl || (e = l.event.special[i.name]._default, l.event.special[i.name]._default = function(t) {
            return !e || !e.apply || t.namespace && -1 !== t.namespace.indexOf("owl") ? t.namespace && -1 < t.namespace.indexOf("owl") : e.apply(this, arguments)
        }, l.event.special[i.name].owl = !0)) : i.type === p.Type.State && (this._states.tags[i.name] ? this._states.tags[i.name] = this._states.tags[i.name].concat(i.tags) : this._states.tags[i.name] = i.tags, this._states.tags[i.name] = l.grep(this._states.tags[i.name], l.proxy(function(t, e) {
            return l.inArray(t, this._states.tags[i.name]) === e
        }, this)))
    }, p.prototype.suppress = function(t) {
        l.each(t, l.proxy(function(t, e) {
            this._supress[e] = !0
        }, this))
    }, p.prototype.release = function(t) {
        l.each(t, l.proxy(function(t, e) {
            delete this._supress[e]
        }, this))
    }, p.prototype.pointer = function(t) {
        var e = {
            x: null,
            y: null
        };
        return (t = (t = t.originalEvent || t || c.event).touches && t.touches.length ? t.touches[0] : t.changedTouches && t.changedTouches.length ? t.changedTouches[0] : t).pageX ? (e.x = t.pageX, e.y = t.pageY) : (e.x = t.clientX, e.y = t.clientY), e
    }, p.prototype.isNumeric = function(t) {
        return !isNaN(parseFloat(t))
    }, p.prototype.difference = function(t, e) {
        return {
            x: t.x - e.x,
            y: t.y - e.y
        }
        ...
        return this.each(function() {
            ...
            i || (i = new p(this, "object" == typeof e && e), t.data("owl.carousel", i), l.each(["next", "prev", "to", "destroy", "refresh", "replace", "add", "remove"], function(t, e) {
                i.register({
                    type: p.Type.Event,
                    name: e
    ...
    }, l.fn.owlCarousel.Constructor = p
